...
## Distributional agreement (1-Wasserstein)

Agreement of posterior *means* is necessary but not sufficient: two
posteriors can share a mean yet differ in spread or shape. We therefore
compare the full marginal distributions with the 1-Wasserstein
(earth-mover's) distance, standardised by the pooled posterior SD so it is
scale-free. As a Monte Carlo reference we also report the distance between
two independent smoothbp runs (seeds 31 and 32); cross-package distances of
the same order as this within-sampler reference indicate marginals that are
indistinguishable up to Monte Carlo error.
